...
(function($) {
    ...
    var win = $(window),
        ...
        // DOM elements
        overlay, closeBtn, image, zoomBtn, prevBtn, nextBtn, leftBtn, rightBtn, bottom, caption, nav, number, deleteBtn,
    ...
    window.curPreviewImg = 0;

    /*
        Initialization
    */
    $(document).ready(function() {
        window.previewBodyReady = function() {
            $(document.body).append(
                $([
                    overlay = $('<div id="pbOverlay" />').click()[0],
                    closeBtn = $('<div id="pbCloseBtn" />')[0],
                    viewerBtn = $('<div title="查看初始状态" id="pbViewerBtn" />').click(doubleClick)[0],
                    image = $('<img id="pbImage" />').dblclick(doubleClick)[0],
                    bottom = $('<div id="pbBottom" />').append([
                        caption = $('<div id="pbCaption" />')[0],
                        $('<div id="pbNav" />').append([
                            zoomInBtn = $('<a id="pbBigBtn" href="#" title="放大" />').click(zoomIn)[0],
                            zoomOutBtn = $('<a id="pbSmallBtn" href="#" title="缩小" />').click(zoomOut)[0],
                            prevBtn = $('<a id="pbPrevBtn" class="" href="#" title="上翻" />').click(previous)[0],
                            //downloadBtn = $('<a id="pbDownloadBtn" href="#" title="下载到本地" />').click()[0],
                            zoomBtn = $('<a id="pbDownloadBtn" href="#" title="下载到本地" />').click()[0],
                            rightBtn = $('<a id="pbRightBtn" href="#" title="右旋" />').click(rotateRight)[0],
                            leftBtn = $('<a id="pbLeftBtn" href="#" title="左旋" />').click(rotateLeft)[0],
                            nextBtn = $('<a id="pbNextBtn" class="" href="#" title="下翻" />').click(next)[0]
                        ])[0],
                        number = $('<div id="pbNumber" />')[0]
                    ])[0]
                ]).css("display", "none")
            );
            deleteBtn = $('.delete-btn').get(0);
            $('#pbCloseBtn,#pbViewerBtn').show();
    ...
    */
    ...
    function mouseMove() {
        flashFade([bottom, prevBtn, zoomBtn, nextBtn, leftBtn, rightBtn, deleteBtn]);
    }
    ...
    function flashFade(targets, out) {
        ...
        newTargets = $.grep(targets, function(target, i) {
    ...
    function previous() {
        window.curPreviewImg = prevImage;
        return changeImage(prevImage, true);
    }

    function next() {
        window.curPreviewImg = nextImage;
    ...
    function showImage(noAnim) {
            ...
            $(image).css("display", "");
            $(image).attr("data-num", activeImage);
            //$(image).rotate(0);
            $(image).rotateImg({
                "angle": 0
            });
    ...
    $.fn.rotateImg = function(options) {
        ...
        this.each(function() {
            ...
            var m11 = Math.cos(rad) * options.scale,
                ...
                m22 = m11;
            var angleStr = options.angle.toString();
            if (iebrowser) {
                $img.css("filter", "progid:DXImageTransform.Microsoft.Matrix(M11=" + m11 + ",M12=" + m12 + ",M21=" + m21 + ",M22=" + m22 + ",SizingMethod='auto expand')");
            } else {
                $img.css({
                    "transform": "rotate(" + angleStr + "deg)",
                    "-ms-transform": "rotate(" + angleStr + "deg)",
                    "-webkit-transform": "rotate(" + angleStr + "deg)",
                    "-o-transform": "rotate(" + angleStr + "deg)",
                    "-moz-transform": "rotate(" + angleStr + "deg)"
                });
            }
        });
    };
